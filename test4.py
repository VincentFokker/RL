...
if __name__ == "__main__":
    ...
    with open(join(specified_path,'Bestmodel_{}'.format(args.name), 'config.yml'), 'r') as f:
    ...
    modelpath = join(specified_path, 'Bestmodel_{}'.format(args.name), 'best_model.zip')
    ...
    for episode in range(10):
        ...
        while not done:
            ...
            if args.render:
                ...
                ax1.imshow(state_n)
                ax1.set_title('State')
                ax2.bar(x=range(0,stop), height=model.action_probability(state))
        ...
        with open(join(specified_path, 'Bestmodel_{}'.format(args.name),'performance_metrics.txt'), 'a') as f:
            f.write("%s\n" % meta_data[-1])