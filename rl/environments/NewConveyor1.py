...
# Version: 1.0
#
#  Should have decisions on where items should go at choice of the agent
#
# run with > python train.py -e NewConveyor1 -s Test -n Test123
# TEST with > python test.py -e NewConveyor1 -s Test -n 0 --render
#
########################################################################################
...
class NewConveyor1(gym.Env):
    ...
    def make_observation(self):
        ...
        1. observe bottom part of conveyor / or only output points? > investigate
        2. queue demand
        3. amount of items in pipeline
        4. TODO: split pipeline and in_queue  / total processing time in pipeline
        ...
        obs = np.append(carrier_type_map_obs, np.array(self.queue_demand))

        # 3 - in pipeline (amounts) for each queue
        in_pipeline = []
        for i in range(1, 3 + 1):
            in_pipeline.append(len([item for item in self.items_on_conv if item[2] == i]))
        obs = np.append(obs, np.array(in_pipeline))

        # 4 - length of queues
        in_queues = [len(item) for item in self.in_queue]
        obs=np.append(obs, np.array(in_queues))
    ...
    def step_env(self):
        ...
        for item in self.items_on_conv:
            if item[0] in self.diverter_locations:
                ...
                if item[1] == self.queue_demand[self.diverter_locations.index(item[0])] and item[2] == (
                        self.diverter_locations.index(item[0]) + 1):
