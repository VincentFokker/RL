...
# run with > python train.py -e simple_conveyor_1 -s conv_test -n test1        #
...
######## INITIALIZATION OF VARIABLES ###############################################################################################################
    def __init__(self, config, *args):
        ...
        self.observation_space = gym.spaces.Discrete(118)
        self.observation_space = gym.spaces.Box(shape=(118, ),
                                                high=200, low=0,
                                                dtype=np.uint8)
...
####### FOR SIMULATION ONLY
        ...
        return self.make_observation().astype(int)
...
####Do step for all items on conv
    ...
    def step(self, action):
        ...
        reward = self.reward
        try:
            if max([item[2] for item in self.items_on_conv]) >= self.max_time_in_system:
                self.terminate = True
        except:
            self.terminate = False
