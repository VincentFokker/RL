...
class NewConveyor12(gym.Env):
    ...
    def __init__(self, config):
        ...
        ##### shape
        #1      amount_of_outputs
        #2      3*self.in_que_observed
        #3      amount_of_gtps
        #4      amount_of_gtps
        #5      1
        #6      1
        self.shape = self.amount_of_outputs + 3*self.in_que_observed + self.amount_of_gtps + self.amount_of_gtps + 1
    ...
    def make_observation(self):
        ...
        # 5 - cycle count ##############################################################################################
        cyclecount = self.cycle_count / self.max_cycle_count
        obs = np.append(obs, np.array(cyclecount))
    ...
    def process_at_GTP(self):
        ...
        for Transition_point in O_locs:  # For all operator locations, check:
            ...
            # this should not happen, but a condition to fix in case it does occur:
                ...
                self.W_times[O_locs.index(Transition_point) + 1] = 0
