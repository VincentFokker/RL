...
class NewConveyor7(gym.Env):
    ...
    def __init__(self, config):
        ...
        self.pipeline_reward = self.config['pipeline_reward']

        # init variables
        self.episode = 0
        self.reward = 0
        self.terminate = False
        self.items_on_conv = [] # [[x, y] , type, goal]
        ...
        self.shape = self.amount_of_outputs + 3*self.in_que_observed + 2 * self.amount_of_gtps + self.amount_of_gtps
    ...
    def make_observation(self):
        ...
        ## 3 - 2 vars: queue can take items, queue is empty
        cantake = []
        isempty = []
        for queue in self.in_queue:
            floatwaarde = len(queue) / 7  #2 = 0, tot aan 7 = 1
            if len(queue) < 7:
                cantake.append(1)
            elif len(queue) == 7:
                cantake.append(0)
            if len(queue) < 2:
                ...
                isempty.append(0)
        queue = np.append(np.array(cantake), np.array(isempty))
        obs = np.append(obs, queue)
    ...
    def step(self, action):
        ...
        for loc in self.diverter_locations:
            ...
            if amount_in_pipe > self.optimal_pipe:
                self.reward += self.pipeline_reward
            elif amount_in_pipe > 2*self.optimal_pipe:
                self.reward += self.pipeline_reward * 2
            elif amount_in_pipe > 3*self.optimal_pipe:
                self.reward += self.pipeline_reward * 3
