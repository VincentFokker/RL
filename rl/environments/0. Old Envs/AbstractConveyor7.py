...
class AbstractConveyor7(gym.Env):
...
######## INITIALIZATION OF VARIABLES ###############################################################################################################
    def __init__(self, config, **kwargs):
        ...
        self.additional_terminations = self.config['additional_terminations']
        ...
        self.reward_1_delivery_ep = self.config['reward_1_delivery_ep']
        self.reward_2_num_carriers_ep = self.config['reward_2_carriers_ep']
        self.reward_3_num_idletime_ep = self.config['reward_3_idletime_ep']
        self.reward_4_cycles_ep = self.config['reward_4_cycles_ep']

        self.reward_1_delivery = self.config['reward_1_delivery']
        self.reward_2_num_carriers = self.config['reward_2_carriers']
        self.reward_3_num_idletime = self.config['reward_3_idletime']
        self.reward_4_cycles = self.config['reward_4_cycles']

        self.reward_carriers_on_cycle = self.config['reward_carriers_on_cycle']
        self.reward_carriers_in_pipe = self.config['reward_carriers_in_pipe']
        self.reward_queue_len = self.config['reward_queue_len']
        self.reward_too_less_time = self.config['reward_too_less_time']
        self.reward_wrong_terminate_situation = self.config['reward_wrong_terminate_situation']
        self.chosen_wrong_supply = self.config['chosen_wrong_supply']
        self.reward_proper_output = self.config['reward_proper_output']
        ...
        self.operator_locations = [[i, self.empty_env.shape[0] - 3] for i in range(4, self.amount_of_gtps * 4 + 1, 4)][
                                  ::-1]
        self.max_on_conv = 2 * ((self.amount_of_gtps * 4) + self.pipeline_length -1 +2 * self.amount_of_outputs) + 2*4
        ...
        if 15 in self.observation_shape:
            self.shape += 2 * 2 * ((self.amount_of_gtps * 4) + self.pipeline_length - 1 + 2 * self.amount_of_outputs)
        ...
        self.cycle_count = 0
        self.items_delivered_in_queue = 0
        self.action_is_feasible = False
        self.is_in_future_demand = False
        self.not_in_pipe = False
    ...
    def make_observation(self):
        ...
        in_pipe2 = [
            [len([item for item in self.items_on_conv if item[2] == i and item[1] == j and item[0][1] < 8]) for j in
             range(1, self.amount_of_outputs + 1)] for i in range(1, self.amount_of_gtps + 1)]
        in_pipe2 = [item for sublist in in_pipe2 for item in sublist]
        in_pipe2 = [item if item <= 0.5 * self.pipeline_length else 0.5 * self.pipeline_length for item in in_pipe2]
        in_pipe2 = np.array(in_pipe2) / 0.5 * self.pipeline_length
        ...
        self.tot_wait_time = np.array(
            [queue_times[i] + self.W_times['{}'.format(i + 1)] for i in range(self.amount_of_gtps)])
        tot_wait_time = self.tot_wait_time / max_time

        #TODO: return in_pipe2
        ### 15. type and goal bottom conv
        self.carrier_type_map_obs = np.zeros((self.empty_env.shape[0], self.empty_env.shape[1], 1)).astype(float)
        ...
        type_map_obs = self.carrier_type_map_obs[2:8, 1:-1]  # cut padding #for the carrier type
        type_map_obs1 = self.carrier_type_map_obs[2:8, 1:-1]  # cut padding #for the carrier type
        carrier_type_map_obs = type_map_obs[-1]  # Only observe bottom lane #top and bottom lane for the carrier type
        carrier_type_map_obs1 = type_map_obs[-1]  # Only observe bottom lane #top and bottom lane for the carrier type
        type_map_obs = np.array(
            [self.encode(item) for item in list(carrier_type_map_obs)]).flatten()  # binary encoded memory for the type
        type_map_obs1 = np.array(
            [self.encode(item) for item in list(carrier_type_map_obs1)]).flatten()
        typeandgoal = np.append(type_map_obs, type_map_obs1)
        ### 16 distances to goal ####################################################
        distances = []
        for i in range(self.amount_of_gtps):
            cand = [item for item in self.items_on_conv if
                    item[0][1] == 7 and item[0][0] > self.diverter_locations[i][0] and item[1] == self.queue_demand[i] and
                    item[2] == i + 1]
            if cand == []:
                distances.append(1)
            else:
                distances.append((min([c[0][0] for c in cand]) - self.diverter_locations[i][0]) / (
                            self.pipeline_length + 4 * i + 2 * (self.queue_demand[i] - 1)))
        ...
        if 15 in self.observation_shape:
            obs = np.append(obs, typeandgoal)
        if 16 in self.observation_shape:
            obs = np.append(obs, distances)
...
#
    def step_env(self):
        ...
        logging.debug('Set a step in env')
        ...
        for item in self.items_on_conv:
            if item[0] in self.diverter_locations:
                ...
                if condition_1 and condition_2 and condition_3:
                    ...
                    item[0][1] += 1
                    self.items_delivered_in_queue +=1
                    self.reward += self.reward_1_delivery
    ...
    def step(self, action=None, next_O=None, next_D=None):
        logging.debug(self.init_queues)
        #print('step taken {}'.format(action), end='\r')
        self.reward = 0
        self.steps +=1
        ...
        logging.debug('Next O: {} \n Next D: {}'.format(self.next_O, self.next_D))


        # 1. Check if step is feasible:
        self.check_infeasible_action()
        if self.action_is_feasible == False:
            logging.debug('Action is not feasible: \t {}'.format(self.action_is_feasible))

        # 2. check if the chosen action is in future demand at workstation
        if self.action_is_feasible:
            self.carrier_is_in_coming_demand(self.lookahead)
        if not self.is_in_future_demand:
            logging.debug('Is in not future demand: \t {}'.format(self.is_in_future_demand))

        # 3. check if not already in the pipeline somewhere on conveyor
        if self.action_is_feasible and self.is_in_future_demand:
            self.check_if_in_pipe()
        if not self.not_in_pipe:
            logging.debug('Item allready in pipeline: \t {}'.format(self.not_in_pipe))

        # 4. allow action to be performed
        if self.action_is_feasible and self.is_in_future_demand:
            logging.debug('Next O: {}, Next D {}'.format(self.next_O, self.next_D))
            self.step_env()
            ## OUTPUT ITEM ON CONVEYOR
            ol = deepcopy(self.output_locations)
            next_O = deepcopy(self.next_O) - 1
            next_D = deepcopy(self.next_D) -1
            self.items_on_conv.append([ol[next_O], self.next_O, self.next_D])
            logging.debug('Item output at output: {}'.format(self.next_O))
            self.in_pipe[next_D].append(self.next_O)
            self.reward +=self.reward_proper_output
            self.ejected_carriers +=1
            #print("ejected carriers: {}".format(self.ejected_carriers), end='\r')

        # 5. if the action is not feasi
        #if not self.action_is_feasible or not self.not_in_pipe or not self.is_in_future_demand:
        else:
            self.next_O, self.next_D = 0, 0
            self.step_env()
        ...
        if self.additional_terminations:
        #Terminate also when one of these apply:
            if self.cycle_count > self.max_cycle_count:
                self.terminate = True
                self.reward -= self.reward_wrong_terminate_situation
                logging.debug('Terminate because max cycle count exceeded')

            if self.steps > self.max_steps:
                self.terminate = True
                self.reward -= self.reward_wrong_terminate_situation
                logging.debug('Terminate because max steps exceeded')
        ...
        if self.termination_condition == 1:
            if self.demand_queues == [[] * i for i in range(self.amount_of_gtps)]:
                self.terminate = True
                logging.debug('Terminate because demand_queues is empty')

        elif self.termination_condition == 2:
            if self.init_queues == [[] * i for i in range(self.amount_of_gtps)]:
                self.terminate = True
                logging.debug('Terminate because init_queues is empty')

        elif self.termination_condition ==3:
            if self.items_processed > self.max_items_processed:
                self.terminate = True
                logging.debug('Terminate because items_processed > 0 is empty')

        #additional rewards
        for idx, time in enumerate(self.tot_wait_time):
            dist = self.gtp_buffer_length + self.pipeline_length + 4*idx + 2
            diff = -1 * (time - dist)
            if diff >= 0 and diff < 5:
                self.reward-=self.reward_too_less_time
            elif diff >= 5 and diff < 10:
                self.reward-=self.reward_too_less_time*2
            elif diff >= 10 and diff < 15:
                self.reward-=self.reward_too_less_time*3
            elif diff >= 15 and diff < 20:
                self.reward -= self.reward_too_less_time * 3
            elif diff > 20:
                self.reward-=self.reward_too_less_time*4

        # reward for the number of carriers on the conveyor
        percentage_occupied = len([item for item in self.items_on_conv if item[0][1] <= 7]) / (
                    2 * ((self.amount_of_gtps * 4) + self.pipeline_length + 2 + 2 * self.amount_of_outputs + 6))
        if percentage_occupied > 0.05:
            self.reward -= self.reward_2_num_carriers
        elif percentage_occupied > 0.10:
            self.reward -= 2 * self.reward_2_num_carriers
        elif percentage_occupied > 0.15:
            self.reward -= 3 * self.reward_2_num_carriers
        elif percentage_occupied > 0.20:
            self.reward -= 4 * self.reward_2_num_carriers
        elif percentage_occupied > 0.25:
            self.reward -= 5 * self.reward_2_num_carriers
        else:
            self.reward -= 6 * self.reward_2_num_carriers
    ...
    def check_if_in_pipe(self):
        """check if chosen action is not allready in pipeline"""
        if self.next_O not in self.in_pipe[self.next_D-1]:
            self.not_in_pipe = True
        else:
            self.not_in_pipe = False
            self.reward -= self.flooding_reward


    def carrier_is_in_coming_demand(self, lookahead):
        """"Checks of the carrier is in demand at workstation in the near future"""
        if self.next_O in [item[:lookahead] for item in self.init_queues][self.next_D-1]:
            self.is_in_future_demand = True
        else:
            self.is_in_future_demand = False
            self.reward -= self.chosen_wrong_supply



    def check_infeasible_action(self):
        """"Takes the next O, returns if it is possible or not"""
        ol = deepcopy(self.output_locations)
        loc = ol[self.next_O-1]

        self.carrier_type_map = np.zeros((self.empty_env.shape[0], self.empty_env.shape[1], 1)).astype(float)
        for item in self.items_on_conv:
            self.carrier_type_map[item[0][1]][item[0][0]] = item[1]

        if self.carrier_type_map[loc[1]][loc[0]+1] == 0:
            #print('action is feasible')
            self.action_is_feasible = True
