...
class AbstractConveyor6(gym.Env):
...
#
    ...
    def step(self, action=None, next_O=None, next_D=None):
        ...
        if len([item for item in self.items_on_conv if
            ...
            self.reward -= self.reward_4_cycles_ep
        ...
        self.reward -=self.cycle_count_delta * self.reward_3_num_idletime
        ...
        percentage_occupied =len([item for item in self.items_on_conv if item[0][1] <= 7]) / (2*((self.amount_of_gtps * 4) + self.pipeline_length +2 +2 * self.amount_of_outputs+6))
        ...
        #carriers on cycle
        carriers_on_cycle = len([item for item in self.items_on_conv if item[0][1] < 7])
        if carriers_on_cycle > 0:
            self.reward -= self.reward_carriers_on_cycle
        elif carriers_on_cycle > 3:
            self.reward -= self.reward_carriers_on_cycle *2
        elif carriers_on_cycle > 5:
            self.reward -= self.reward_carriers_on_cycle *3
        elif carriers_on_cycle > 10:
            self.reward -= self.reward_carriers_on_cycle *4
        elif carriers_on_cycle > 15:
            self.reward -= self.reward_carriers_on_cycle *5

        # carriers on cycle
        carriers_in_pipe = len([item for item in self.items_on_conv])
        if carriers_in_pipe > 10:
            self.reward -=self.reward_carriers_in_pipe

        #carriers in queue
        for queue in self.len_queues:
            if queue < 0.2:
                self.reward -= self.reward_queue_len
            if queue > .8:
                self.reward -= self.reward_queue_len
        ...
        if self.cycle_count > self.max_cycle_count:
            self.terminate = True
            # rewards calculated
            self.reward += self.reward_1_delivery_ep * self.items_delivered_in_queue
            self.reward -= self.reward_2_num_carriers_ep * len([item for item in self.items_on_conv if item[0][1] <= 7])
            self.reward -= self.reward_3_num_idletime_ep * sum(self.idle_times_operator)
            self.reward -= self.reward_4_cycles_ep * self.cycle_count
        ...
        if self.termination_condition == 1:
            if self.demand_queues == [[] * i for i in range(self.amount_of_gtps)]:
                ...
                # rewards calculated
                self.reward += self.reward_1_delivery_ep * self.items_delivered_in_queue
                self.reward -= self.reward_2_num_carriers_ep * len(
