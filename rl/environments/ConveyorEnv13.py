...
######## INITIALIZATION OF VARIABLES ###############################################################################################################
    def __init__(self, config):
        ...
        self.max_idle  = self.config['max_idle']
    ...
    def step(self, action=None, next_O=None, next_D=None):
        ...
        if self.idle_time_delta != 0:
            ...
            if self.terminate_on_idle:
                #self.reward += self.reward_towards_goal*(self.items_processed / self.max_items_processed))
                self.reward -= self.reward_towards_goal * ((self.max_items_processed - self.items_processed) / self.max_items_processed)
                self.terminate = True
