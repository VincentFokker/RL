...
class SimpleConveyor1GA(gym.Env):
...
######## INITIALIZATION OF VARIABLES ###############################################################################################################
    def __init__(self, config, queues, **kwargs):
        ...
        self.action_space = gym.spaces.Discrete(self.amount_of_outputs+1)
        ...
        self.positive_reward_for_output  = self.config['positive_reward_for_output']
...
#
    def make_observation(self):
        ...
        if 1 in bottom_conv:
            info.append(1)
            info.append(len([item for item in bottom_conv if item == 1]) / 2*(
                        (self.amount_of_gtps * 4) + 6 + 7 + 2 * self.amount_of_outputs))
        else:
            info.append(0)
            info.append(0)
        ...
        if 2 in bottom_conv:
            info.append(1)
            info.append(len([item for item in bottom_conv if item == 2]) / 2* (
                        (self.amount_of_gtps * 4) + 6 + 7 + 2 * self.amount_of_outputs))
        else:
            info.append(0)
            info.append(0)
...
####toggle diverters if needed
        ...
        for loc2 in d_locs:
            try:
                ...
                logging.debug('Another error occurred; this should not happen! Investigate the cause!')
                self.D_states[d_locs.index(loc2)+1] = False
...
####Do step for all items on conv
        for item in self.items_on_conv:
            ...
            if item[0] in self.diverter_locations:
                try:
                    ...
                    logging.debug('condition 1 = {}, and condition 2= {}'.format(cond_1, cond_2))
                    if cond_1 and cond_2:
                        ...
                        item[0][0] -=1
                        self.reward += self.punishment_for_not_taken
                except:
        ...
        for cord2 in self.output_locations:
            ...
            if self.O_states[self.output_locations.index(loc)+1] !=0 and self.carrier_type_map[cord2[1]][cord2[0]+1] ==0:
                ...
                self.reward += self.positive_reward_for_output
