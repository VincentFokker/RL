...
# Version: 5.1                                                                         #
#                                           #
# Includes WARM START! based on 4 includes different introduction policy              #
        ...
        for loc2 in d_locs:
            try:
                ...
                if carrier_map[loc2[1]][loc2[0]] != 0:
                    condition_2 = len(self.in_queue[d_locs.index(loc2)]) <= min(map(len, [self.in_queue[self.init_queues.index(i)] for i in [item for item in self.init_queues if item[0] == self.init_queues[d_locs.index(loc2)][0]]]))
                else:
                    condition_2 = False
                logging.info('Condition 2 == {}'.format(condition_2))
                if condition_2 == True:
